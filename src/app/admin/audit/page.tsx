...
export default async function AdminAuditPage() {
  ...
  return (
    <>
      ...
      <p className="mt-2 max-w-2xl text-sm leading-6 text-[#77727f]">
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[720px] text-left text-sm">
            ...
            <tbody>
              {(data ?? []).map((log) => {
                ...
                return (
                  <tr key={log.id} className="border-b border-black/5 last:border-0 align-top">
                    <td className="whitespace-nowrap px-5 py-4 text-[#8a8491]">
                      {new Date(log.created_at).toLocaleString()}
                    </td>
                    <td className="px-5 py-4">
                      <p className="font-bold">{actor?.display_name ?? "Unknown"}</p>
                      <p className="mt-0.5 text-[11px] text-[#9a95a0]">
                    ...
                    <td className="px-5 py-4">
                      <p>{log.entity}</p>
                      <p className="mt-0.5 font-mono text-[11px] text-[#9a95a0]">
                        {log.entity_id ?? "—"}
                      </p>
                    </td>
                    <td className="max-w-xs px-5 py-4 text-xs leading-5 text-[#77727f]">
                      {meta || "—"}
                    </td>
                  </tr>
                );
              ...
              {!data?.length && (
                <tr>
                  <td colSpan={5} className="px-5 py-10 text-center text-[#9a95a0]">
                    No audit events yet.
                  </td>
                </tr>
              )}
