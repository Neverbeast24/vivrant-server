...
const modules: Module[] = [
  ...
  },
  {
    icon: Dumbbell,
    title: "Training",
    copy: "Daily activity and gym — workouts, demos, machines, and AI plans together.",
    tag: "Train",
  ...
  },
  {
    icon: Refrigerator,
    title: "Kitchen",
    copy: "Shopping list and pantry stock in one kitchen flow.",
    tag: "Plan",
  ...
  },
  {
    icon: HeartPulse,
    title: "Wellness",
    copy: "Sleep, hydration, and mood check-ins from one daily rhythm.",
    tag: "Recover",
    iconClass: "bg-gradient-to-br from-[#64748b] to-[#94a3b8]",
  },
  {
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          >
            <div className="flex items-start justify-between">
              <span className="grid size-11 place-items-center rounded-2xl bg-gradient-to-br from-accent-deep to-cyan text-accent-fg shadow-lg shadow-accent/25 transition-transform duration-300 group-hover:scale-110 group-hover:rotate-6">
                <Sparkles size={19} />
              </span>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {testimonials.map((item, index) => (
            <motion.article
              ...
              whileHover={{ y: -6 }}
              className="relative overflow-hidden rounded-[1.7rem] border border-ink/12 bg-card p-6 shadow-[0_16px_40px_rgba(var(--shadow-color),.06)]"
            >
              <div className={`absolute -right-8 -top-8 size-32 rounded-full bg-gradient-to-br ${item.accent} opacity-20 blur-2xl`} />
              <p className="relative text-sm leading-7 text-ink/85">&ldquo;{item.quote}&rdquo;</p>
              <div className="relative mt-6 flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[11px] font-semibold text-ink/65">{item.role}</p>
                </div>
              </div>
            </motion.article>
          ))}
